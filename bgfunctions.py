...
driver.implicitly_wait(4) #allow time for webpage to load
...
def scrape(inputurl, year):
    ...
    if cond == 0:
        ...
        for q in a:
            ...
            cur.execute('INSERT INTO [Table Names]([Table Name], Columns, Value, Panda) VALUES (?,?,?,?)', (overall, tableheader1, values, pandas))
            conn.commit()
    else:
        # est = []
        # dd = list(define)
